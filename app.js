...
function sortear() {
  ...
  if (isNaN(quantidade) || isNaN(de) || isNaN(ate)) {
    resultado.innerHTML =
      '<label class="texto__paragrafo">Preencha <strong>todos os campos</strong> acima para realizar o sorteio!</label>';

    desaativarBotão(botaoSortear);
    ativarBotão(botaoReiniciar);
  } else if (quantidade > numerosDisponíveis) {
    resultado.innerHTML =
      '<label class="texto__paragrafo">A quantidade de números sorteados precisa ser <strong>MENOR</strong> que a quantidade de números disponíveis para sorteio</label>';

    desaativarBotão(botaoSortear);
    ativarBotão(botaoReiniciar);
  } else if (quantidade == 0) {
    resultado.innerHTML =
      '<label class="texto__paragrafo">A quantidade de números sorteados <strong>NÃO PODE</strong> ser igual a ZERO!</label>';

    desaativarBotão(botaoSortear);
    ativarBotão(botaoReiniciar);
  } else if (de >= ate) {
    resultado.innerHTML =
      '<label class="texto__paragrafo">O valor em <strong>"Do número"</strong> precisa ser <strong>MENOR</strong> que o valor em <strong>Até o número</strong></label>';
